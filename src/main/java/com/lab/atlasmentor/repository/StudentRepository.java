...
public interface StudentRepository extends JpaRepository<Student, Long> {
    ...
    @Query(value = "SELECT s.* FROM students s LEFT JOIN users u ON s.user_id = u.id WHERE s.assigned_by_id = :counsellorId AND (:status IS NULL OR s.status = :status) AND (LOWER(u.first_name) LIKE :search OR LOWER(u.last_name) LIKE :search OR LOWER(s.email) LIKE :search OR u.phone LIKE :search OR u IS NULL)", nativeQuery = true)
    ...
    // Methods backing GET /api/students/non-registered - despite the name, this now returns
    // students of every status (including REGISTERED); the optional :status param is the only
    // way to narrow it to one status. The name is kept to avoid an unrelated churn of renaming
    // the endpoint/controller/service method - see conversation for context.
    @Query(value = "SELECT s.* FROM students s LEFT JOIN users u ON s.user_id = u.id LEFT JOIN countries c ON s.country_id = c.id LEFT JOIN users creator ON s.created_by = creator.id LEFT JOIN roles r ON creator.role_id = r.id WHERE (:isAdmin = true OR s.branch_id IS NULL OR s.branch_id = :branchId) AND (:status IS NULL OR s.status = :status) AND (:countryName IS NULL OR LOWER(c.name) LIKE LOWER(CONCAT('%', :countryName, '%'))) AND (:dateFrom IS NULL OR s.created_at >= CAST(:dateFrom AS timestamp)) AND (:dateTo IS NULL OR s.created_at <= CAST(:dateTo AS timestamp)) AND (:source IS NULL OR (r.name IN (:sourceRoles) OR (s.source_type IN (:sourceTypes) AND s.source_type IS NOT NULL))) AND (LOWER(u.first_name) LIKE :search OR LOWER(u.last_name) LIKE :search OR LOWER(s.email) LIKE :search OR u.phone LIKE :search OR u IS NULL)", nativeQuery = true)
    ...
    @Query(value = "SELECT s.* FROM students s LEFT JOIN users u ON s.user_id = u.id LEFT JOIN countries c ON s.country_id = c.id LEFT JOIN users creator ON s.created_by = creator.id LEFT JOIN roles r ON creator.role_id = r.id WHERE s.assigned_by_id = :counsellorId AND (:status IS NULL OR s.status = :status) AND (:countryName IS NULL OR LOWER(c.name) LIKE LOWER(CONCAT('%', :countryName, '%'))) AND (:dateFrom IS NULL OR s.created_at >= CAST(:dateFrom AS timestamp)) AND (:dateTo IS NULL OR s.created_at <= CAST(:dateTo AS timestamp)) AND (:source IS NULL OR (r.name IN (:sourceRoles) OR (s.source_type IN (:sourceTypes) AND s.source_type IS NOT NULL))) AND (LOWER(u.first_name) LIKE :search OR LOWER(u.last_name) LIKE :search OR LOWER(s.email) LIKE :search OR u.phone LIKE :search OR u IS NULL)", nativeQuery = true)
    ...
    @Query(value = "SELECT s.* FROM students s LEFT JOIN users u ON s.user_id = u.id LEFT JOIN countries c ON s.country_id = c.id LEFT JOIN users creator ON s.created_by = creator.id LEFT JOIN roles r ON creator.role_id = r.id WHERE s.created_by = :creatorId AND (:status IS NULL OR s.status = :status) AND (:countryName IS NULL OR LOWER(c.name) LIKE LOWER(CONCAT('%', :countryName, '%'))) AND (:dateFrom IS NULL OR s.created_at >= CAST(:dateFrom AS timestamp)) AND (:dateTo IS NULL OR s.created_at <= CAST(:dateTo AS timestamp)) AND (:source IS NULL OR (r.name IN (:sourceRoles) OR (s.source_type IN (:sourceTypes) AND s.source_type IS NOT NULL))) AND (LOWER(u.first_name) LIKE :search OR LOWER(u.last_name) LIKE :search OR LOWER(s.email) LIKE :search OR s.phone LIKE :search OR u IS NULL)", nativeQuery = true)
